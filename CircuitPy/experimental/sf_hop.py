...
def find_spreading_factor():
    for spreading_factor in spreading_factors:
        ...
        if packet is not None:
            print(
                f"Received packet on spreading factor: {spreading_factor}: {packet}, RSSI: {radio1.last_rssi-137}"
            )
        else:
...
def respond_to_ping():
    ...
    while True:
        ...
        if packet is not None:
            print(
                f"Received packet on spreading factor: {spreading_factor}: {packet}, RSSI: {radio1.last_rssi}"
            )
