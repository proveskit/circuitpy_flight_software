...
class CanBusHelper:
        ...
        for i, message in enumerate(messages):
            ...
            while attempts < 3 and not ack_received:
                try:
                    ...
                    ack_received = self.wait_for_ack(expected_ack_id=message.id, timeout=1.0)
                    if not ack_received:
                        attempts += 1
                        self.debug_print(f"ACK not received for message {i}. Attempt {attempts}")
                except Exception as e:
                    self.debug_print("Error Sending data over CAN bus" + ''.join(traceback.format_exception(None, e, e.__traceback__)))
                    break

            if not ack_received:
                self.debug_print(f"Failed to receive ACK after {attempts} attempts for message {i}.")
                return False
        ...
        with self.can_bus.listen(timeout=timeout) as listener:
            ...
            for _ in range(message_count):
                try:
                    ...
                    self.debug_print("Error processing message: " + ''.join(traceback.format_exception(None, e, e.__traceback__)))
    ...
    def send_ack(self, msg_id, is_extended=False):
        ...
        try:
            ...
            self.debug_print("Error sending ACK: " + ''.join(traceback.format_exception(None, e, e.__traceback__)))
    ...
    def handle_multi_message(self, msg):
        ...
        self.debug_print(f"Received multi-message chunk for ID: {original_id} with sequence number: {sequence_number}")

        if str(original_id) in self.multi_message_buffer and not self.multi_message_buffer[str(original_id)]['is_complete']:
            # Store this chunk in the buffer
            self.multi_message_buffer[str(original_id)]['received_chunks'][sequence_number] = msg.data
            ...
            self.debug_print(f"Unexpected multi-message chunk received for ID: {original_id}")
    ...
    def reassemble_message(self, original_id):
        ...
        """
        buffer = self.multi_message_buffer[str(original_id)]
        complete_message = b''.join(buffer['received_chunks'][seq] for seq in sorted(buffer['received_chunks']))
    ...
    def send_sot(self, original_id, data_length):
        ...
        try:
            ...
            self.debug_print(f"Sent SOT for ID: {sot_id} with data length: {data_length}")
        except Exception as e:
            self.debug_print("Error sending SOT: " + ''.join(traceback.format_exception(None, e, e.__traceback__)))
    ...
    def handle_sot_message(self, msg):
        ...
        try:
            original_id, data_length = msg.data.decode('utf-8').split(':')  # Assuming data is sent as a string
            data_length = int(data_length)
            self.debug_print(f"Received SOT for ID: {msg.id} with expected data length: {data_length}")
        except ValueError:
    ...
    def send_eot(self):
        ...
        try:
            ...
            self.debug_print("Error sending EOT: " + ''.join(traceback.format_exception(None, e, e.__traceback__)))
    ...
    def send_rtr_and_receive(self, rtr_id, timeout=5.0):
        ...
        try:
            ...
            self.debug_print("Error sending RTR: " + ''.join(traceback.format_exception(None, e, e.__traceback__)))
            return None
