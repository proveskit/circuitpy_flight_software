...
class functions:
    ...
    def __init__(self,cubesat):
        ...
        try:
            ...
            self.debug_print("Couldn't turn faces on: " + ''.join(traceback.format_exception(e))) #pylint: disable=no-value-for-parameter
    ...
    def battery_heater(self):
        ...
        try:
            try:
                ...
                self.debug_print("[ERROR] couldn't get thermocouple data!" + ''.join(traceback.format_exception(e))) #pylint: disable=no-value-for-parameter
            ...
            self.debug_print("Error Initiating Battery Heater" + ''.join(traceback.format_exception(e))) #pylint: disable=no-value-for-parameter
            return False
        finally:
    ...
    def battery_manager(self):
        ...
        try:
            ...
            self.debug_print(f'Internal Temperature: {self.cubesat.internal_temperature} C')
        except Exception as e:
            self.debug_print("Error obtaining battery data: " + ''.join(traceback.format_exception(e))) #pylint: disable=no-value-for-parameter
        ...
        try:
            ...
            if vbatt < self.cubesat.CRITICAL_BATTERY_VOLTAGE:
                ...
                self.powermode('low')
                self.debug_print('CONTEXT SHIFT INTO LOW POWER MODE: Attempting to shutdown unnecessary systems...')

            elif vbatt > self.cubesat.NORMAL_BATTERY_VOLTAGE+.5:
                self.powermode('max')
                self.debug_print('CONTEXT SHIFT INTO MAXIMUM POWER MODE: Attempting to revive all systems...')

            elif vbatt < self.cubesat.NORMAL_BATTERY_VOLTAGE+.3 and self.cubesat.power_mode=='maximum': #We are passing data in a weird way here, please check
                self.powermode('norm')
                self.debug_print('CONTEXT SHIFT INTO NORMAL POWER MODE: Attempting to revive necessary systems...')

        except Exception as e:
            self.debug_print("Error in Battery Manager: " + ''.join(traceback.format_exception(e))) #pylint: disable=no-value-for-parameter
    ...
    def powermode(self,mode):
        ...
        try:
            ...
            self.debug_print("Error in changing operations of powermode: " + ''.join(traceback.format_exception(e))) #pylint: disable=no-value-for-parameter
    ...
    def state_of_health(self):
        ...
        try:
            ...
            self.debug_print("Couldn't aquire data for the state of health: " + ''.join(traceback.format_exception(e))) #pylint: disable=no-value-for-parameter
        try:
