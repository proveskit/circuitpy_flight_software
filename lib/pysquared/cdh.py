...
class CommandDataHandler:
    ...
    def passcode_equals_super_secret_code(self, cubesat: Satellite, msg):
        if msg[3] & 0x08:
            multi_msg = True
        # strip off RH header
        msg = bytes(msg[4:])
        ...
        try:
            ...
            self.logger.error("There was an error decoding the arguments", err=e)

        return cmd, cmd_args, multi_msg

    def valid_command(self, cubesat: Satellite, msg, cmd, cmd_args):
        try:
            if cmd_args is None:
                self.logger.info(
    ...
    # [RH header 4 bytes] [pass-code(4 bytes)] [cmd 2 bytes]
    def len_10_or_greater(self, cubesat: Satellite, msg):
        if bytes(msg[4:8]) == self._super_secret_code:
            # check if multi-message flag is set
            cmd, cmd_args, multi_msg = self.passcode_equals_super_secret_code(
                cubesat, msg
            )

        if cmd in self._commands:
            self.valid_command(cubesat, msg, cmd, cmd_args)
            return
    ...
    def message_handler(self, cubesat: Satellite, msg) -> None:
        ...
        if len(msg) >= 10:  # [RH header 4 bytes] [pass-code(4 bytes)] [cmd 2 bytes]
            self.len_10_or_greater(cubesat, msg)
